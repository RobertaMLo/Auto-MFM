...
def plot_TF_numerical_vs_analytical_2D_modspace(MEANfreq, SDfreq, fiSim, Fe_eff_grc, yout_max, xlim, w,
                                                     P, alpha, params, xname, barname, FOLDER, facW):
        ...
        #ax_cb = plt.subplot2grid((1, 16), (0, 15)) #more cols in (,) == colorbar more stretta

        # -- Setting up a colormap that's a simple transtion
        mymap = plt.cm.viridis

        
        # #  I removed this complicated function -- this is still useful to map exaclti the value of numerical tf into colors
        #build_bar_legend(np.round(levels, 1), levels.size, ax_cb, mymap,
        #                 label=barname + ' ($\\nu_i$)' + ' [Hz]')  # BUILD THE COLOR SCALE BAR


        # Needed to have a function customizable for all populations
        vmin = fiSim.min()
        vmax = fiSim.max()

        # Needed to associate right solor to right line
        norm = Normalize(vmin=vmin, vmax=vmax)
        sm = plt.cm.ScalarMappable(cmap=mymap, norm=norm)
        sm.set_array([])  # obbligatorio per creare la colorbar
        ...
        for i in range(levels.size):
            ...
            #### plot dei pallini - template numerico
            ax.errorbar(feSim[cond], SIMvector[cond], yerr=SDvector[cond], \
                        color=mymap(r, 1), marker='D', ms=2, capsize=1.5, elinewidth=1, lw=0)
