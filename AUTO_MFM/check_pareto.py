...
def plot_pareto_with_selection(mse, slope, idx_best, alphas_best, method, threshold=0.05, save_fig = False):
    """
    Plot for papers
    """
    fig, ax = plt.subplots(figsize=(2.5, 2))
    ax.scatter(mse, slope, c="black", alpha=0.6, s = 20, marker= 'o', label="Pareto solutions")

    # Evidenzia il punto selezionato
    ax.scatter(mse[idx_best], slope[idx_best],
                c="red", s=60, label="Selected solution",
                edgecolors="red", marker = 's', linewidths=0.99, zorder=5)

    alpha_str = ", ".join([f"{a:.2f}" for a in alphas_best])
    ax.set_title(f"α = [{alpha_str}]", fontsize=9)
...
npz_path = "/run/user/1000/gvfs/sftp:host=90.147.108.122,user=neuro_img/home/neuro_img/Users/Roberta/cerebellum_zplus_plv_ie480_corrected/final_pareto_front_bello_NEW_CONFIG_52450.npz"
...
if os.path.exists(npz_path):
    ...
    plot_pareto_with_selection(mse_filtered, slope_filtered, idx_best, sol_best, method, threshold=thrs, save_fig=True)
