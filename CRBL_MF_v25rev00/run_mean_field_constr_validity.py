...
def mf_snn_io_relation(MF_avg, MF_std, SNN_avg, SNN_std, fmossy, outdir, pop_name, color):
    ...
    plt.xlabel('Mossy fibers [Hz]', fontsize = 12)
...
if __name__ == '__main__':
    ...
    outdir = path_mf+'output_constr_validity'
    ...
    for i in fmossy:
        ...
        PC_mf = np.load(os.path.join(path_mf,str(i)+'.0_CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red','PC_TOT_AVG_SD.npy'), allow_pickle = True)
        MLI_mf = np.load(os.path.join(path_mf,str(i)+'.0_CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red','MLI_TOT_AVG_SD.npy'),allow_pickle = True)
        GoC_mf = np.load(os.path.join(path_mf,str(i)+'.0_CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red','GoC_TOT_AVG_SD.npy'), allow_pickle = True)
        GrC_mf= np.load(os.path.join(path_mf,str(i)+'.0_CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red','GrC_TOT_AVG_SD.npy'), allow_pickle = True)
    ...
    csv_file_mli = os.path.join(path_snn, 'MLI_firing_rate_stats.csv')
    ...
    mli_snn.append([ df_mli.iloc[:, 1], df_mli.iloc[:, 2] ]) #NON HO CALCOLATO DEV STD QUINDI METTO DENTRO UNA COLONNA UGUALE.
    ...
    mf_snn_io_relation(PC_arr[:,0], PC_arr[:,1], pc_snn_arr[0,:], pc_snn_arr[1,:], fmossy, outdir = outdir, pop_name='PC', color='green')
