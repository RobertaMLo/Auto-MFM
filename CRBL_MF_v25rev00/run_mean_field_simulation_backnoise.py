...
def plot_MF_activity_withSD(t, X, finput, mytitle, outdir, font_size = 22, linew=1.5):
    ...
    ax2.fill_between(t, X[:, 9] + np.sqrt(abs(X[:, 11])), X[:, 9] - np.sqrt(abs(X[:, 11])), facecolor='orange', alpha=0.5)
    ...
    plt.savefig(outdir+'/'+'mf_activity.png', dpi = 300, bbox_inches='tight') #bbox added to save the figure correctly
    #plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-NTWK", help="synaptic and connectivity properties",
                        default = 'CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red')

    parser.add_argument('-f_backnoise', type=float, 
                        default=4., help="Backgound noise rate [Hz], default at 4 Hz")
    
    parser.add_argument('-alfa', default=[2., 1.7, 1.8, 8.], #fist trial [2., 1.7, 3., 10.]
                        help="alpha for prediction [GrC GoC MLI PC]")
    
    parser.add_argument('-sim_len', type=float, default=5., 
    ...
    FILE_GrC = root_path  + '20250212_111423_GrC_CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red_tsim5_alpha2.0_fit.npy'
    FILE_GoC = root_path + '20250212_124057_GoC_CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red_tsim5_alpha1.7_fit.npy'
    FILE_MLI = root_path + '20250206_160547_MLI_CRBL_CONFIG_20PARALLEL_wN_PLOS23_tsim5_alpha1.8_fit.npy'
    FILE_PC = root_path + '20250212_111659_PC_CRBL_CONFIG_20PARALLEL_wN_PLOS23_Kgocgrc_red_tsim5_alpha2.7_fit.npy' #Zmin
    ...
    if args.save_sim:
        ...
        np.save(outdir+'/alphas.npy', args.alfa)
