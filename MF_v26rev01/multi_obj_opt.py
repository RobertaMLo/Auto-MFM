...
class SynapseOptimization(Problem):
    def __init__(
        ...
        self.tmax = tmax
        super().__init__(
            n_var=len(param_names), #number of variables I wanna optimzie
            n_obj=2,
            n_constr=1,
    ...
    def _evaluate(self, X, out, *args, **kwargs): #Here I put the objective function.
        f1 = []
        f2 = []
        for params in X:
            for name, value in zip(self.param_names, params):
                self.cell_params[name] = value # These value for me it is alpha when I want to optimize alpha
                #single_sim = simulation I wanna optimize
            times, g_syn = single_sim(
                self.cell_params, self.sim_params, self.E_rev, duration=self.duration
            )
          # Objective function -- take as input params and provides as output the error
